...
**Számok:** pozitív gold a teljes top-5-ben: **6/8**; átrendezés (nyers vs. teljes top-1):
**6**; negatív teszt: **2/2 absztenció** (mindkettő üres → „nincs információm").

> A HyDE generálás sztochasztikus, ezért a pozitív találati arány futásonként ingadozik
> (mértünk 4/8 és 6/8 közötti értékeket ugyanezzel a `qwen2.5:3b` konfigurációval); a fenti
> a legjobb helyi futás. A negatív absztenció (2/2) és az átrendezés (≥6) minden futásban stabil.
...
- **A HyDE + rerank értéke egyértelmű, amikor a HyDE jó.** Ahol a kis modell értelmes angol
  HyDE-t írt (Catan, Carcassonne, Ticket to Ride, 7 Wonders, Azul, King of Tokyo), ott a gold
  a teljes pipeline top-1–2 helyére került, és a távolság drámaian javult (nyers ~0.43–0.48 →
  teljes ~0.16–0.29). A nyers (magyar kérdés → angol korpusz, HyDE nélkül) baseline
  következetesen gyenge (a gold jellemzően a top-5-en kívül).
- **Miért 6/8 (és nem 8/8)?** A szűk keresztmetszet a **HyDE minősége a 3B modellen**: a két
  bukó esetben a HyDE elromlott — `splendor-win`-nél tárgyi tévedést írt (a „20 presztízspont",
  valójában 15), `pandemic-lose`-nál pedig a rendszerprompt ellenére magyarul, értelmetlenül
  generált („A Pokémon: The Cure…"), ami elrontja a keresztnyelvi vektort. A rossz HyDE rossz
  vagy üres találatot hoz. Ez **nem a pipeline hibája**, hanem a helyi kis modell korlátja:
  erős válasz-/HyDE-modellel (élesben Claude/GPT) a 6/8 érdemben feljebb vihető. A 6 dokumentált
  átrendezés így is bőven teljesíti az SM-2 célt.
