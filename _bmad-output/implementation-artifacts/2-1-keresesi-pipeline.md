...
- [x] [Review][Patch] A `retrieve` eldobta a modellhívások usage-ét — az AD-11 (minden modellhívás usage-e naplózandó) sérült. `RetrievalTrace.usage` bevezetve, a HyDE + embed + rerank token összegzve (+ teszt). Forrás: acceptance (Med). [retrieve.ts]
- [x] [Review][Patch] A `rerank` az érvénytelen-kimenet fallback-ágán 0 tokent jelentett, pedig a hívás elköltötte a tokeneket — a valós usage megőrizve (csak a dobott hívásnál marad 0) (+ teszt). Forrás: edge (Low). [rerank.ts]
- [x] [Review][Defer] A `retrieve` kontextusa a `[game · section]` prefixet a chunk-fejléc mellé teszi (kis redundancia) — a válasz-modell (Story 2.2) számára hasznos jelzés; nem javítjuk.
...
- 2026-07-18: Code review — 2 patch (retrieve usage-aggregálás + rerank usage-megőrzés, AD-11), a tesztek frissítve, kapu zöld (115+1 skip). 1 defer (kontextus-prefix). Status → done.
